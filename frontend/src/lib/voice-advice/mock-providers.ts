// Mock providers for the voice / AI advice / TTS stream.
// Server-side uses the same logic via api/v1/_lib/mock-providers.ts.
...
import { SAFETY_COPY, buildSafeFallbackAdvice, classifySafety, cryptoRandomUUID, isProhibited, synthesizeEscalation, validateAdviceOutput } from "./safety";
...
export function scenarioToTranscriptResult(input: {
  ...
  let cursor = 0;
  const segments: TranscriptSegment[] = input.scenario.transcript.map((line) => {
...
export function generateMockAdvice(input: {
  ...
  const adviceItems = safety.level === "amber" ? [] : buildAdviceItems(input.checkin, input.transcriptText);
  ...
  const candidate: AdviceOutput = {
    ...
    followUpQuestion:
      safety.level === "amber"
        ? null
        : typeof input.checkin.sleepMinutes === "number" && input.checkin.sleepMinutes < 360
...
function buildObservations(checkin: HealthCheckin, transcriptText: string, safety: SafetyClassification) {
  ...
  if (safety.reasonCodes.includes("diagnosis_request")) {
    observations.push({
      statement: "Your question asks for a clinical assessment, which SleepOS does not provide.",
      evidenceMetricKeys: [],
...
function buildAdviceItems(checkin: HealthCheckin, transcriptText: string): AdviceOutput["adviceItems"] {
  ...
  if (items.length === 0 && transcriptText.trim().length > 0) {
    items.push({
...
function buildSummary(checkin: HealthCheckin, safety: SafetyClassification): string {
